...
public class PlayerListener implements Listener {
    private List<InventoryType> bannedInventoryTypes = Arrays.asList(
    ...
    private List<String> allowedInventoryTypes = Arrays.asList(
            "&e&lSpectate &f▸ &7(Right click)",
            "&e&lSpectate Settings &f▸ &7(Right click)",
            "&e&lPlay Again &f▸ &7(Right click)",
            "&c&lBack to Lobby &f▸ &7(Right click)"
    );

    private List<XMaterial> bannedInteraction = Arrays.asList(
      XMaterial.ANVIL,
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        XMaterial xMaterial = XMaterial.matchXMaterial(new ItemStack(material));
        ...
        Game game = plugin.getGm().getGameByPlayer(p);
        if(UltraSkyWarsAPI.isSpectator(p) && !UltraSkyWarsAPI.isPlayerGame(p)) {
            if(e.getAction().equals(Action.LEFT_CLICK_BLOCK) && bannedInteraction.contains(xMaterial)) {
                e.setCancelled(true);
            }
        }
    ...
    public void onDamage(EntityDamageEvent e) {
        ...
        if (ent instanceof Player) {
            ...
            UltraSkyWars plugin = UltraSkyWars.get();
            Game g = plugin.getGm().getGameByPlayer(p);
            GamePlayer gamePlayer = g.getGamePlayerByName(p.getName());
            if (g == null) {
                return;
            }
    ...
    @EventHandler
    public void onInteractEvent(InventoryOpenEvent event) {
        Inventory inventory = event.getInventory();
        HumanEntity player = event.getPlayer();

        if (UltraSkyWarsAPI.isSpectator((Player) player) && !UltraSkyWarsAPI.isPlayerGame((Player) player)) {
            if (bannedInventoryTypes.contains(inventory) && !allowedInventoryTypes.contains(inventory)) {
