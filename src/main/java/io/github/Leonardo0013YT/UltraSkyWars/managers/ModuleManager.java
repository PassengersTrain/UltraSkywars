...
public class ModuleManager {
    ...
    public ModuleManager() {
        ...
        File modules = new File(plugin.getDataFolder(), "modules");
        if(!modules.exists()) {
            modules.mkdir();
        }
    }
    ...
    public void loadWEInjection() {
        ...
        UltraSkyWars plugin = UltraSkyWars.get();
        File modules = new File(plugin.getDataFolder(), "modules");
            we = new File(modules, "UltraSkyWars-WEOld.jar");
    ...
    public void reload() {
        UltraSkyWars plugin = UltraSkyWars.get();
        File modules = new File(plugin.getDataFolder(), "modules");
        File litems = new File(modules, "UltraSkyWars-SpecialItems.jar");
        ...
        File lsoulwell = new File(modules, "UltraSkyWars-SoulWell.jar");
        if (soulwell == null) {
            if (lsoulwell.exists()) {
        ...
        File lpartys = new File(modules, "UltraSkyWars-Parties.jar");
        if (party == null) {
            if (lpartys.exists()) {
                loadJarFile(lpartys);
                party = new InjectionParty();
        ...
        File lchallenges = new File(modules, "UltraSkyWars-Challenges.jar");
        if (challenges == null) {
        ...
        File lprotect = new File(modules, "UltraSkyWars-LobbyProtect.jar");
        if (lProtection == null) {
            if (lprotect.exists()) {
        ...
        File pwt = new File(modules, "UltraSkyWars-PWT.jar");
        if (pwts == null) {
            if (pwt.exists()) {
                loadJarFile(pwt);
                pwts = new InjectionPWT();
        ...
        File mob = new File(modules, "UltraSkyWars-MobFriends.jar");
        if (mobFriends == null) {
            if (mob.exists()) {
                loadJarFile(mob);
                mobFriends = new InjectionMobFriends();
        ...
        File sign = new File(modules, "UltraSkyWars-Signs.jar");
        if (signs == null) {
            if (sign.exists()) {
                loadJarFile(sign);
                signs = new InjectionSigns();
        ...
        File eloRanks = new File(modules, "UltraSkyWars-EloRanks.jar");
        if (eloRank == null) {
            if (eloRanks.exists()) {
        ...
        File perk = new File(modules, "UltraSkyWars-Perks.jar");
        if (perks == null) {
            if (perk.exists()) {
                loadJarFile(perk);
                perks = new InjectionPerks();
        ...
        File cubelet = new File(modules, "UltraSkyWars-Cubelets.jar");
        if (cubelets == null) {
            if (cubelet.exists()) {
                loadJarFile(cubelet);
                cubelets = new InjectionCubelets();
